...
const mockRepository = {
  getBalance: vi.fn<(userId: string) => Promise<number | undefined>>(() =>
    Promise.resolve(undefined),
  ),
  initializeBalance: vi.fn<(userId: string, balance: number) => Promise<TokenBalance>>(() =>
    Promise.resolve({} as TokenBalance),
  ),
  creditTokens: vi.fn<(userId: string, amount: number) => Promise<number>>(() =>
    Promise.resolve(0),
  ),
  debitToken: vi.fn<(userId: string) => Promise<number | null>>(() => Promise.resolve(null)),
  recordTransaction: vi.fn<(data: unknown) => Promise<TokenTransaction>>(() =>
    Promise.resolve({} as TokenTransaction),
  ),
  getTransactions: vi.fn<
    (userId: string, limit: number, offset: number) => Promise<TokenTransaction[]>
  >(() => Promise.resolve([])),
  getTransactionCount: vi.fn<(userId: string) => Promise<number>>(() => Promise.resolve(0)),
};
