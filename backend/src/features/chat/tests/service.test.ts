...
const mockRepository = {
  findConversationById: vi.fn<(id: string) => Promise<Conversation | undefined>>(() =>
    Promise.resolve(undefined),
  ),
  createConversation: vi.fn<(data: unknown) => Promise<Conversation>>(() =>
    Promise.resolve({} as Conversation),
  ),
  updateConversation: vi.fn<(id: string, data: unknown) => Promise<Conversation | undefined>>(() =>
    Promise.resolve(undefined),
  ),
  deleteConversation: vi.fn<(id: string) => Promise<boolean>>(() => Promise.resolve(false)),
  findMessagesByConversationId: vi.fn<
    (conversationId: string, limit?: number) => Promise<Message[]>
  >(() => Promise.resolve([])),
  createMessage: vi.fn<(data: unknown) => Promise<Message>>(() => Promise.resolve({} as Message)),
};
